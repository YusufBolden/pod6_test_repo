...
def test_get_titles(playlist):
	expected = ["Killing Me Softly", "Four Women", "Mothership Connection (Star Child)",
	"Unfunky UFO", "Sometimes", "Mississippi Goddamn", "Another Brick in the Wall"]
...
def test_search_killing_me(playlist):
	expected = [
		{'artist': 'Fugees', 'title': 'Kiling Me Soflty'},
	]
	actual = search_by_title(playlist, "Killing Me Softly")
	assert actual == expected

def test_search_brick_wall(playlist):
	expected = [
		{'artist': 'Pink Floyd', 'title': 'Another Brick in the Wall'},
    ]
	actual = search_by_title(playlist, "Killing Me Softly")
